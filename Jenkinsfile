        ...
        stage('Deploy') {
            steps {
                sshagent(credentials: ['webapp-ssh-key']) {
                    ...
                    ssh ${DEPLOY_USER}@${DEPLOY_HOST} 'docker build -t ${IMAGE_NAME}:${IMAGE_TAG} /home/${DEPLOY_USER}/ && docker run -d --network flask-net -p 5000:5000 --name ${IMAGE_NAME} ${IMAGE_NAME}:${IMAGE_TAG}'
                    """
                }
            }
        }
